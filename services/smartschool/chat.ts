import { Mail, Recipients, SmartSchool } from "smartschooljs";

import { error } from "@/utils/logger/logger";

import { Chat, Message, Recipient } from "../shared/chat";

export async function fetchSkolengoChats(session: SmartSchool, accountId: string): Promise<Chat[]> {
...
export async function fetchSkolengoAvailableRecipients(session: SmartSchool): Promise<Recipient[]> {
...
export async function createSkolengoMail(session: SmartSchool, accountId: string, subject: string, content: string, recipients: Recipient[], cc?: Recipient[], bcc?: Recipient[]): Promise<Chat> {
