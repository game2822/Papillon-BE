...
import { isValidInstance } from "smartschooljs";
import Reanimated, {
  Extrapolate,
  interpolate,
  useAnimatedStyle,
...
export default function URLInputScreen() {
  ...
  let CleanedinstanceURL = instanceURL.trim();
  ...
  const onValidate = async () => {
    ...
    if (!instanceURL.includes("https")) {
      CleanedinstanceURL = "https://" + instanceURL;
    } else {
      CleanedinstanceURL = instanceURL;
    }
    const isvalid = await isValidInstance(CleanedinstanceURL);

    if (isvalid) {
      return router.push({
        pathname: "./webview" as unknown as RelativePathString,
        params: { url: CleanedinstanceURL },
      });
    } else {
      return alert.showAlert({
        title: t("ONBOARDING_URL_INVALID_TITLE"),
        description: t("ONBOARDING_URL_INVALID_DESCRIPTION"),
        icon: "TriangleAlert",
        color: "#D60046",
        withoutNavbar: true,
      });
    ...
    >
      <ViewContainer>
        ...
        <Reanimated.View style={AnimatedInputContainerStyle}>
          <OnboardingInput
            placeholder={t("ONBOARDING_URL_SMARTSCHOOL_PLACEHOLDER")}
            icon={"Link"}
            text={instanceURL}
